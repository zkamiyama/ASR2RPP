...
同梱TOMLは展開フォルダーの `models/` を直接読み、コピー・同期は行いません。
独自定義はユーザーデータの `custom-models/`（旧 `models/` の独自IDも互換読込）。
旧同梱コピーは既知SHA-256で識別して無視し、同名編集済みファイルは競合を通知します。
ユーザーファイルは移動・上書き・削除しません。既存GGMLは再ダウンロード不要です。
